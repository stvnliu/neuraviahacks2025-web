<script lang="ts">
    ...
    async function getData(type: string, span: string): Promise<{xLabels: string[]; yLabels: number[]}> {
        // const data: Data = await (await fetch(`${backend}/data?type=${type}&span=${span}`, {
        //     mode: 'cors',
        //     headers: {
        ...
        const xs: string[] = [];
        const ys: number[] = [];
        data.data.forEach((point) => {xs.push(point.x); ys.push(point.y)});
